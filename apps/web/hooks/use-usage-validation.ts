...
import { validateFileAgainstLimits, isUnlimited } from "@/lib/usage-tracking-client";
...
export interface UseUsageValidationReturn {
  validateFilesForUpload: (files: File[]) => Promise<UsageValidationResult | null>;
...
export function useUsageValidation(): UseUsageValidationReturn {
  ...
  const validateFilesForUpload = useCallback(async (files: File[]): Promise<UsageValidationResult | null> => {
    ...
    try {
      ...
      if (isUnlimited(currentUsage.documents.limit) && isUnlimited(currentUsage.storage.limit)) {
        return {
          canUpload: true,
          allowedFiles: files,
          rejectedFiles: [],
      ...
      const upgradeRequired = hasRejectedFiles && (
        (!isUnlimited(currentUsage.documents.limit) && currentUsage.documents.used >= currentUsage.documents.limit) ||
        (!isUnlimited(currentUsage.storage.limit) && storageUsed > currentUsage.storage.limit)
      );
      ...
      const errorMessage = error instanceof Error ? error.message : "Failed to validate usage limits";
